import psycopg2
from config import data_base, host, user, password
...
def insert(surname, name, phone):
    cur.execute("SELECT count(*) FROM phonebook1 WHERE surname='{}' AND name='{}'".format(surname, name))
    if cur.fetchone()[0]==0:
         cur.execute("""INSERT INTO phonebook1 VALUES ('{}','{}', {})""".format(surname, name, phone))
    else:
         cur.execute("""UPDATE phonebook1
         SET number={}
...
def delete():
    query="""DELETE FROM phone_book_2
    WHERE """
    cur.execute("SELECT * from phone_book_2")
    print(cur.fetchall())
